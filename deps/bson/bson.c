...
const char* bson_first_errormsg(bson *b) {
    ...
    } else if (b->err & BSON_SIZE_OVERFLOW) {
        return "BSON size overflow";
    } else if (b->err) {
        return "Unspecified BSON error";
    } else {
        return "BSON no error";
    }
}
...
#if 0   /* Disabled because it is not used */
...
const char* bson_iterator_pointer(bson_iterator *i) {
    return i->cur;
}
...
bson_type bson_find_fieldpath_value3(FFPCTX* ffctx) {
    ...
    if (ffctx->fplen <= BSON_MAX_FPATH_LEN) {
        ...
        pstack = bson_malloc_func((ffctx->fplen + 1) * sizeof (char));
        if (!pstack) {
            return BSON_EOO;
        }
    }
...
void bson_init_buffer(bson *b, char *buf, int max) {
    bson_reset(b);
    b->data = buf;
    b->cur = b->data + 4;
    b->dataSize = max;
    if (b->data == buf) {
        b->flags |= BSON_FLAG_EXTERNAL_BUFFER;
    }
}
...
int bson_append_string_maxlen(bson *b, const char *name, const char *value, int max) {
    int len = strnlen(value, max);
    len = (len == max ? max - 1 : len);
    return bson_append_string_base(b, name, value, len, BSON_STRING);
}
...
int bson_append_binary(bson *b, const char *name, const char *str, int len) {
    if (bson_append_estart(b, BSON_BINDATA, name, 4 + 1 + len) == BSON_ERROR)
        return BSON_ERROR;
    bson_append32(b, &len);
    bson_append_byte(b, BSON_BIN_BINARY);
...
int bson_numstrn(char *str, int maxbuf, int64_t i) {
    return bson_sprintf(str, "%d" , (int64_t) i);
}
...
#if 0   /* Disabled because function is not being used */
static void bson_append_fpath_from_iterator(const char *fpath, const bson_iterator *from, bson *into) {
...
#if 0 // disabled TC dependent code
...
#if 0 // depends on bson_compare_it_current, which uses TC for comparing strings
...
#if 0 // depends on bson_compare_fpaths which in turn...
...
#if 0 // depends on bson_compare_it_current, which depends on a TC based string comparison
...
static bson_visitor_cmd_t _bson_remove_visitor(
        const char *ipath, int ipathlen, 
        const char *key, int keylen,
        const bson_iterator *it, 
        bool after, void *op) 
{
    ...
    if (!ctx->data) {
        //WISHDEBUG(LOG_CRITICAL, "remove iterator: %s (%s) %s %s", tpath, tkey, bson_type_string(bt), after ? "after" : "before");
    }
    
    if (!ctx->data && !after && strcmp(tpath, ctx->path) == 0) {
        //WISHDEBUG(LOG_CRITICAL, "removing this path: %s (%s) %s %s", tpath, tkey, bson_type_string(bt), after ? "after" : "before");
        return BSON_VCMD_SKIP_NESTED | BSON_VCMD_SKIP_AFTER;
    }
...
static void bson_append_into_inner(const bson* from, bson* to, const char* path, const char* string) {
    ...
    _BSONMERGE3CTX ctx = {
        ...
        .path = path,
        .data = string,
        .type = string ? BSON_STRING : BSON_EOO,
        .lastkey = NULL,
        .lastindex = 0,
        .nstack = 0
    };
...
static void bson_remove_inner(const bson* from, bson* to, const char* path, const char* string) {
    ...
    bson_visit_fields(&i, 0, _bson_remove_visitor, &ctx);
}

/**
 * Copy bson object with complate state
 * 
 * Copies complete data, state and errors to output object
 * 
 * @param out
 * @param in
 * @return 
 */
static int bson_copy_with_state(bson* out, const bson* in) {
    if (!out || !in) {
        return BSON_ERROR;
    }
    ...
    // copy bson data
    memcpy(out->data, in->data, bson_size(in));
    bson_init_finished_data(out, out->data);
    
    // save the original buffer
    char* data = out->data;
    int dataSize = out->dataSize;
    
    // copy state
    memcpy(out, in, sizeof(bson));
...
void bson_remove_string(bson* bs, const char* path, const char* string) {
    ...
    // Do the inserting
    bson_remove_inner(bs, &tmp, path, string);
...
void bson_remove_path(bson* bs, const char* path) {
    ...
    // Do the inserting
    bson_remove_inner(bs, &tmp, path, NULL);
